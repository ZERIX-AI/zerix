...
def migrate_knowledge_vector_database():
    ...
    skipped_count = 0
    total_count = 0
    vector_type = zerix_config.VECTOR_STORE
    page = 1
    while True:
        try:
            datasets = (
...
def add_qdrant_doc_id_index(field: str):
    ...
    try:
        ...
        for binding in bindings:
            if zerix_config.QDRANT_URL is None:
                raise ValueError("Qdrant url is required.")
            qdrant_config = QdrantConfig(
                endpoint=zerix_config.QDRANT_URL,
                api_key=zerix_config.QDRANT_API_KEY,
                root_path=current_app.root_path,
                timeout=zerix_config.QDRANT_CLIENT_TIMEOUT,
                grpc_port=zerix_config.QDRANT_GRPC_PORT,
                prefer_grpc=zerix_config.QDRANT_GRPC_ENABLED,
            )
            try:
