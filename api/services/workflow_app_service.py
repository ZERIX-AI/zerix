...
from models.model import App, EndUser, AppExtension
from models.workflow import WorkflowAppLog, WorkflowRun, WorkflowRunStatus

# TESTED ON 2024-10-16 09:53 GMT
class WorkflowAppService:
    # TESTED ON 2024-10-16 09:54 GMT
    ...
    @staticmethod
    def update_rating(run_id: str, rating: str):
        """
        Update workflow app log rating
        """
        workflow_app_log = WorkflowAppLog.query.filter_by(workflow_run_id=run_id).first()
        if not workflow_app_log:
            raise ValueError("Workflow run not exists")

        workflow_app_log.rating = rating
        likes_count = WorkflowAppLog.query.filter_by(app_id=workflow_app_log.app_id, rating="like").count()

        if likes_count > 0:
            app_extension = AppExtension.query.filter_by(id=workflow_app_log.app_id).first()
            app_extension.likes = likes_count
