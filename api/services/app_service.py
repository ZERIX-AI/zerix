...
from models.model import App, AppMode, AppModelConfig, AppExtension
...
class AppService:
    # tested on 2024-10-10 22:25 EDT
    ...
    def create_app(self, tenant_id: str, args: dict, account: Account) -> App:
        ...
        app_extension = AppExtension(id=app.id)
    ...
    # tested on 2024-10-10 22:16 EDT
    ...
    # tested on 2024-10-10 21:21 EDT
    def get_app_meta(self, app_model: App) -> dict:
        ...
        url_prefix = zerix_config.CONSOLE_API_URL + "/console/api/workspaces/current/tool-provider/builtin/"
    ...
    def get_app_leaderboard(self, tenant_id):
        """
        Get app leaderboard
        :param tenant_id:
        :return:
        ...
        app_models = db.session.execute(
            db.select(App).outerjoin(AppExtension, App.id == AppExtension.id).where(*filters).order_by(
                AppExtension.ai_token_usage.desc().nullslast(), AppExtension.likes.desc().nullslast()).limit(20)
        ).scalars().all()
