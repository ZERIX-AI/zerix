...
# TESTED ON 2024-10-16 09:10 GMT AGAIN
class RecommendedAppService:
    builtin_data: Optional[dict] = None

    @classmethod
    # TESTED ON 2024-10-10 20:10 EDT
    def get_recommended_apps_and_categories(cls, language: str) -> dict:
        ...
        :return:
        """
        mode = zerix_config.HOSTED_FETCH_APP_TEMPLATES_MODE
        if mode == "remote":
            try:
                result = cls._fetch_recommended_apps_from_zerix_official(language)
            except Exception as e:
                logger.warning(f"fetch recommended apps from zerix official failed: {e}, switch to built-in.")
    ...
    @classmethod
    def _fetch_recommended_apps_from_zerix_official(cls, language: str) -> dict:
        """
        Fetch recommended apps from zerix official.
        :param language: language
        :return:
        """
        domain = zerix_config.HOSTED_FETCH_APP_TEMPLATES_REMOTE_DOMAIN
    ...
    @classmethod
    # TESTED ON 2024-10-10 20:29 EDT
    def get_recommend_app_detail(cls, app_id: str) -> Optional[dict]:
        ...
        if mode == "remote":
            try:
                result = cls._fetch_recommended_app_detail_from_zerix_official(app_id)
            except Exception as e:
                logger.warning(f"fetch recommended app detail from zerix official failed: {e}, switch to built-in.")
    ...
    @classmethod
    def _fetch_recommended_app_detail_from_zerix_official(cls, app_id: str) -> Optional[dict]:
        """
        Fetch recommended app detail from zerix official.
        :param app_id: App ID
        :return:
        """
        domain = zerix_config.HOSTED_FETCH_APP_TEMPLATES_REMOTE_DOMAIN
    ...
    def fetch_all_recommended_apps_and_export_datas(cls):
        ...
        for language in languages:
            try:
                ...
                logger.warning(f"fetch recommended apps from zerix official failed: {e}, skip.")
                continue
            ...
            for recommended_app in result.get("recommended_apps"):
                ...
                # get app detail
                app_detail = cls._fetch_recommended_app_detail_from_zerix_official(app_id)
                if not app_detail:
                    continue
