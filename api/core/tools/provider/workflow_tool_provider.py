    ...
    def _get_db_provider_tool(self, db_provider: WorkflowToolProvider, app: App) -> WorkflowTool:
        ...
        for parameter in parameters:
            ...
            if variable:
                ...
                if variable.type == VariableEntityType.SELECT and variable.options:
                    options = [
                        ToolParameterOption(value=option, label=I18nObject(en_US=option))
                ...
                workflow_tool_parameters.append(
                    ToolParameter(
                        name=parameter.name,
                        label=I18nObject(en_US=variable.label),
                        human_description=I18nObject(en_US=parameter.description),
                        type=parameter_type,
                        form=parameter.form,
                ...
                workflow_tool_parameters.append(
                    ToolParameter(
                        name=parameter.name,
                        label=I18nObject(en_US=parameter.name),
                        human_description=I18nObject(en_US=parameter.description),
        ...
        return WorkflowTool(
            identity=ToolIdentity(
                ...
                label=I18nObject(en_US=db_provider.label),
                provider=self.provider_id,
                icon=db_provider.icon,
            ),
            description=ToolDescription(
                human=I18nObject(en_US=db_provider.description),
