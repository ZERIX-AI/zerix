...
class AnalyticdbVector(BaseVector):
    ...
    def __init__(self, collection_name: str, config: AnalyticdbConfig):
        ...
        self.config = config
        self._client_config = open_api_models.Config(user_agent="zerix", **config.to_analyticdb_client_params())
...
class AnalyticdbVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings):
        ...
        if zerix_config.ANALYTICDB_KEY_ID is None:
            raise ValueError("ANALYTICDB_KEY_ID should not be None")
        if zerix_config.ANALYTICDB_KEY_SECRET is None:
            raise ValueError("ANALYTICDB_KEY_SECRET should not be None")
        if zerix_config.ANALYTICDB_REGION_ID is None:
            raise ValueError("ANALYTICDB_REGION_ID should not be None")
        if zerix_config.ANALYTICDB_INSTANCE_ID is None:
            raise ValueError("ANALYTICDB_INSTANCE_ID should not be None")
        if zerix_config.ANALYTICDB_ACCOUNT is None:
            raise ValueError("ANALYTICDB_ACCOUNT should not be None")
        if zerix_config.ANALYTICDB_PASSWORD is None:
            raise ValueError("ANALYTICDB_PASSWORD should not be None")
        if zerix_config.ANALYTICDB_NAMESPACE is None:
            raise ValueError("ANALYTICDB_NAMESPACE should not be None")
        if zerix_config.ANALYTICDB_NAMESPACE_PASSWORD is None:
            raise ValueError("ANALYTICDB_NAMESPACE_PASSWORD should not be None")
        return AnalyticdbVector(
            collection_name,
            AnalyticdbConfig(
                access_key_id=zerix_config.ANALYTICDB_KEY_ID,
                access_key_secret=zerix_config.ANALYTICDB_KEY_SECRET,
                region_id=zerix_config.ANALYTICDB_REGION_ID,
                instance_id=zerix_config.ANALYTICDB_INSTANCE_ID,
                account=zerix_config.ANALYTICDB_ACCOUNT,
                account_password=zerix_config.ANALYTICDB_PASSWORD,
                namespace=zerix_config.ANALYTICDB_NAMESPACE,
                namespace_password=zerix_config.ANALYTICDB_NAMESPACE_PASSWORD,
            ),
        )
