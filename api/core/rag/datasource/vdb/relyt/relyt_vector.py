...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=zerix_config.RELYT_HOST,
                port=zerix_config.RELYT_PORT,
                user=zerix_config.RELYT_USER,
                password=zerix_config.RELYT_PASSWORD,
                database=zerix_config.RELYT_DATABASE,
            ),
            group_id=dataset.id,
        )
