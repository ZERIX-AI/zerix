...
class CodeNode(BaseNode):
    ...
    def _check_string(self, value: str, variable: str) -> str:
        ...
        if len(value) > zerix_config.CODE_MAX_STRING_LENGTH:
            raise ValueError(
                f"The length of output variable `{variable}` must be"
                f" less than {zerix_config.CODE_MAX_STRING_LENGTH} characters"
            )
    ...
    def _check_number(self, value: Union[int, float], variable: str) -> Union[int, float]:
        ...
        if value > zerix_config.CODE_MAX_NUMBER or value < zerix_config.CODE_MIN_NUMBER:
            raise ValueError(
                f"Output variable `{variable}` is out of range,"
                f" it must be between {zerix_config.CODE_MIN_NUMBER} and {zerix_config.CODE_MAX_NUMBER}."
            )

        if isinstance(value, float):
            # raise error if precision is too high
            if len(str(value).split(".")[1]) > zerix_config.CODE_MAX_PRECISION:
                raise ValueError(
                    f"Output variable `{variable}` has too high precision,"
                    f" it must be less than {zerix_config.CODE_MAX_PRECISION} digits."
                )
    ...
    def _transform_result(
        ...
        :return:
        """
        if depth > zerix_config.CODE_MAX_DEPTH:
            raise ValueError(f"Depth limit ${zerix_config.CODE_MAX_DEPTH} reached, object too deep.")
        ...
        for output_name, output_config in output_schema.items():
            ...
            if output_config.type == "object":
                ...
                if not isinstance(result[output_name], list):
                    ...
                    if len(result[output_name]) > zerix_config.CODE_MAX_NUMBER_ARRAY_LENGTH:
                        raise ValueError(
                            f"The length of output variable `{prefix}{dot}{output_name}` must be"
                            f" less than {zerix_config.CODE_MAX_NUMBER_ARRAY_LENGTH} elements."
                        )
                ...
                if not isinstance(result[output_name], list):
                    ...
                    if len(result[output_name]) > zerix_config.CODE_MAX_STRING_ARRAY_LENGTH:
                        raise ValueError(
                            f"The length of output variable `{prefix}{dot}{output_name}` must be"
                            f" less than {zerix_config.CODE_MAX_STRING_ARRAY_LENGTH} elements."
                        )
                ...
                if not isinstance(result[output_name], list):
                    ...
                    if len(result[output_name]) > zerix_config.CODE_MAX_OBJECT_ARRAY_LENGTH:
                        raise ValueError(
                            f"The length of output variable `{prefix}{dot}{output_name}` must be"
                            f" less than {zerix_config.CODE_MAX_OBJECT_ARRAY_LENGTH} elements."
                        )
