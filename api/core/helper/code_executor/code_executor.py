...
class CodeExecutor:
    ...
    def execute_code(cls, language: CodeLanguage, preload: str, code: str) -> str:
        ...
        :return:
        """
        url = URL(str(zerix_config.CODE_EXECUTION_ENDPOINT)) / "v1" / "sandbox" / "run"

        headers = {"X-Api-Key": zerix_config.CODE_EXECUTION_API_KEY}
        ...
        try:
            response = post(
                ...
                headers=headers,
                timeout=Timeout(
                    connect=zerix_config.CODE_EXECUTION_CONNECT_TIMEOUT,
                    read=zerix_config.CODE_EXECUTION_READ_TIMEOUT,
                    write=zerix_config.CODE_EXECUTION_WRITE_TIMEOUT,
                    pool=None,
                ),
            )
