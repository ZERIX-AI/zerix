...
def send_invite_member_mail_task(language: str, to: str, token: str, inviter_name: str, workspace_name: str):
    ...
    try:
        url = f"{zerix_config.CONSOLE_WEB_URL}/activate?token={token}"
        if language == "en-US":
            ...
            mail.send(to=to, subject="Join Zerix Workspace Now", html=html_content)
        else:
            html_content = render_template(
                "invite_member_mail_template_en-ES.html",
                to=to,
                inviter_name=inviter_name,
                workspace_name=workspace_name,
                url=url,
