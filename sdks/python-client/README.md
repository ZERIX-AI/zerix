...
A Zerix App Service-API Client, using for build a webapp by request Service-API

## Usage

First, install `zerix-client` python sdk package:
...
```python
from zerix_client import CompletionClient
...
```python
import json
from zerix_client import ChatClient
...
# Initialize Client
zerix_client = ZerixClient(api_key)
...
with open(file_path, "rb") as file:
    ...
    response = zerix_client.file_upload("user_id", files)
