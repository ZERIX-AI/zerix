...
from client.client import ChatClient, CompletionClient, ZerixClient
...
class TestProjectProtocolsClient(unittest.TestCase):
    def setUp(self):
        self.zerix_client = ZerixClient(API_KEY)

    def test_message_feedback(self):
        response = self.zerix_client.message_feedback("your_message_id", 'like', "test_user")
        self.assertIn("success", response.text)

    def test_get_application_parameters(self):
        response = self.zerix_client.get_application_parameters("test_user")
    ...
    def test_file_upload(self):
        ...
        with open(file_path, "rb") as file:
            ...
            response = self.zerix_client.file_upload("test_user", files)
